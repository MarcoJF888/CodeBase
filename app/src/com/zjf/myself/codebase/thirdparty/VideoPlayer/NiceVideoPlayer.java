package com.zjf.myself.codebase.thirdparty.VideoPlayer;
...
public class NiceVideoPlayer extends FrameLayout
    ...
    public void restart() {
        if (mCurrentState == STATE_PAUSED) {
            ...
            AppLog.d("STATE_PLAYING");
        }
        if (mCurrentState == STATE_BUFFERING_PAUSED) {
            mMediaPlayer.start();
            mCurrentState = STATE_BUFFERING_PLAYING;
            mController.setControllerState(mPlayerState, mCurrentState);
            AppLog.d("STATE_BUFFERING_PLAYING");
        }
    }
    ...
    public void pause() {
        if (mCurrentState == STATE_PLAYING) {
            ...
            mController.setControllerState(mPlayerState, mCurrentState);
            AppLog.d("STATE_PAUSED");
        }
        if (mCurrentState == STATE_BUFFERING_PLAYING) {
            mMediaPlayer.pause();
            mCurrentState = STATE_BUFFERING_PAUSED;
            mController.setControllerState(mPlayerState, mCurrentState);
            AppLog.d("STATE_BUFFERING_PAUSED");
        }
    }
    ...
    private void openMediaPlayer() {
        try {
            ...
            mController.setControllerState(mPlayerState, mCurrentState);
            AppLog.d("STATE_PREPARING");
        } catch (IOException e) {
            e.printStackTrace();
            AppLog.e("打开播放器发生错误", e);
        }
    }
    ...
    private MediaPlayer.OnPreparedListener mOnPreparedListener
        ...
        public void onPrepared(MediaPlayer mp) {
            ...
            mController.setControllerState(mPlayerState, mCurrentState);
            AppLog.d("onPrepared ——> STATE_PREPARED");
        }
    };
    ...
    private MediaPlayer.OnVideoSizeChangedListener mOnVideoSizeChangedListener
        ...
        public void onVideoSizeChanged(MediaPlayer mp, int width, int height) {
            AppLog.d("onVideoSizeChanged ——> width：" + width + "，height：" + height);
        }
    };
    ...
    private MediaPlayer.OnCompletionListener mOnCompletionListener
        ...
        public void onCompletion(MediaPlayer mp) {
            mCurrentState = STATE_COMPLETED;
            mController.setControllerState(mPlayerState, mCurrentState);
            AppLog.d("onCompletion ——> STATE_COMPLETED");
    ...
    private MediaPlayer.OnErrorListener mOnErrorListener
        ...
        public boolean onError(MediaPlayer mp, int what, int extra) {
            mCurrentState = STATE_ERROR;
            mController.setControllerState(mPlayerState, mCurrentState);
            AppLog.d("onError ——> STATE_ERROR ———— what：" + what);
            return false;
        }
    };
    ...
    private MediaPlayer.OnInfoListener mOnInfoListener
        ...
        public boolean onInfo(MediaPlayer mp, int what, int extra) {
            if (what == MediaPlayer.MEDIA_INFO_VIDEO_RENDERING_START) {
                ...
                mCurrentState = STATE_PLAYING;
                mController.setControllerState(mPlayerState, mCurrentState);
                AppLog.d("onInfo ——> MEDIA_INFO_VIDEO_RENDERING_START：STATE_PLAYING");
            } else if (what == MediaPlayer.MEDIA_INFO_BUFFERING_START) {
                // MediaPlayer暂时不播放，以缓冲更多的数据
                if (mCurrentState == STATE_PAUSED || mCurrentState == STATE_BUFFERING_PAUSED) {
                    mCurrentState = STATE_BUFFERING_PAUSED;
                    AppLog.d("onInfo ——> MEDIA_INFO_BUFFERING_START：STATE_BUFFERING_PAUSED");
                } else {
                    mCurrentState = STATE_BUFFERING_PLAYING;
                    AppLog.d("onInfo ——> MEDIA_INFO_BUFFERING_START：STATE_BUFFERING_PLAYING");
                }
                mController.setControllerState(mPlayerState, mCurrentState);
                ...
                if (mCurrentState == STATE_BUFFERING_PLAYING) {
                    mCurrentState = STATE_PLAYING;
                    mController.setControllerState(mPlayerState, mCurrentState);
                    AppLog.d("onInfo ——> MEDIA_INFO_BUFFERING_END： STATE_PLAYING");
                }
                if (mCurrentState == STATE_BUFFERING_PAUSED) {
                    mCurrentState = STATE_PAUSED;
                    mController.setControllerState(mPlayerState, mCurrentState);
                    AppLog.d("onInfo ——> MEDIA_INFO_BUFFERING_END： STATE_PAUSED");
                }
            } else {
                AppLog.d("onInfo ——> what：" + what);
            }
            return true;
        }
    };
